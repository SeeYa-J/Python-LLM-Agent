...
#
#         @app.middleware("http")
#         async def itcode_middleware(request: Request, call_next):
#             # 从 header 获取 itcode
#             itcode = request.headers.get("x-user-id")
#             if not itcode and not match_any_pattern(request.url.path, Server.exclude_paths):
#                 return JSONResponse(status_code=401, content={"path": request.url.path,"message": "Missing x-user-id header"})
#                 # itcode = "test_user" # 或者设置为默认值
...
#             "http://localhost:3002",
#             "https://rq-agent-dev.t-sy-in.earth.xcloud.lenovo.com",  # 开发环境前端域名
#             "http://rq-agent-dev.t-sy-in.earth.xcloud.lenovo.com",
#             localHost,
#         ]
#
#         app.add_middleware(
