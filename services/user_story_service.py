...
class UserStoryService:
    ...
    def update_user_story(self, uuid: str, story_data: dict) -> Optional[BizUserStory]:
        """更新用户故事 - 通过新建副本并更新的方式"""
        ...
        self.user_story_dao.update(current_story)
        ...
        return self.user_story_dao.create(new_story)
    ...
    def exhibit_user_story(self,conversation_id:int) -> list[dict]:
        return self.user_story_dao.find_user_story_by_conversation_id(conversation_id)

    def batch_delete_selected_stories(self,uuids:List[str]) -> int:
        """批量软删除uuids对应的用户故事"""
        story_ids = self.user_story_dao.get_story_ids_by_uuids(uuids)
        deleted_count = 0
        for story_id in story_ids:
            if self.user_story_dao.soft_delete(story_id):
                deleted_count += 1

        return deleted_count

    def export_to_excel(self, uuids: List[str]) -> tuple[str, str]:
        """导出用户故事到Excel"""
        stories = self.user_story_dao.find_user_story_by_uuids(uuids)
        # 生成文件
    ...
    def get_latest_user_story_by_conversation_id(self,conversation_id: int)-> List[BizUserStory]:
        """根据conversation_id获取最新的user stories"""
        return self.user_story_dao.query_user_story_by_conversation_id_and_order(conversation_id)
    ...
    def get_user_story_by_json(self,user_story_str: str,user_story_summary: str=None) -> list[dict]:
        ...
        for user_story in user_stories:
            ...
            for key,value in user_story.items():
                ...
                data[new_key] = value
            # TBD数据
            data["jira_assignee"] = current_itcode.get()
    ...
    def save_user_story_v2(self,conversation_id: int,round_number: int,user_story_list: List[dict],operator: str=None)->List[BizUserStory]:
        """保存对话过程中的user_story"""
        result = []
        if round_number==1:
            # 第一轮对话批量插入user_story
            ...
            for story_data in user_story_list:
                ...
                biz_user_story = self.user_story_dao.create(user_story)
            ...
            for story_data in user_story_list:
                ...
                if story_data["uuid"]!="":
                    ...
                    new_user_story = self.user_story_dao.update(last_user_story)
                    ...
                    biz_user_story = self.user_story_dao.create(story_data)
                    result.append(biz_user_story)
                else:
                    story_data["conversation_id"] = conversation_id
                    story_data["version"] = -1
                    story_data["uuid"] = str(uuid.uuid1())
                    user_stories = self.user_story_dao.query_user_story_by_conversation_id_and_order(conversation_id)
